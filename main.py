...
class TradingBot:
    ...
    def _run_symbol(self, symbol: str, equity: float, dd: float):
        ...
        mag_prob    = self.ml[symbol].predict_magnitude(features)
        ...
        scalp_thresh = getattr(config, 'SYMBOL_SCALP_THRESHOLDS', {}).get(
            symbol, getattr(config, 'SCALP_BUY_THRESHOLD', 0.60)
        )
        ...
        # -----------------------------------------------------------------
        # EXECUTION BLOCK WITH REGIME GUARD
        # -----------------------------------------------------------------
